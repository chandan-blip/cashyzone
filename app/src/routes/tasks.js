...
router.get('/:id', optionalAuth, async (req, res, next) => {
  try {
    ...
    if (req.user) {
      const [rows] = await pool.query(
        'SELECT status FROM task_purchases WHERE user_id = ? AND task_id = ?',
        [req.user.id, task.id]
      );
      if (rows.length) status = rows[0].status;
    }
    res.json({ ...task, reward: rewardFor(task.price), status, completed: status === 'completed', purchased: status !== 'none' });
  } catch (err) {
    next(err);
  }
});
...
router.post('/:id/claim', authRequired, async (req, res, next) => {
  ...
  try {
    ...
    await conn.query(
      "UPDATE task_purchases SET status = 'completed', completed_at = NOW() WHERE id = ?",
      [rows[0].id]
    );
    ...
    await conn.commit();
    res.json({ reward, balance: u[0].balance, accuracy: score });
  } catch (err) {
    await conn.rollback();
    next(err);
  } finally {
